...
{
    public async Task CalculateVolumeAlert2(bool recalculateAll, CancellationToken cToken)
    {
        var field = CalcField.VolumeAlert2;
        var toProcess = recalculateAll
            ...
            : await _db.HistoricalData
                .Include(x => x.CalculatedFields)
                .Where(x => x.CalculatedFields == null || string.IsNullOrEmpty(x.CalculatedFields.VolumeAlert2))
                .ToListAsync(cToken);
        ...
        foreach (var entry in toProcess)
        {
            try
            {
                ...
                {
                    entry.CalculatedFields.VolumeAlert2 = "missingdata";
                    updatedEntries++;
                    continue;
                }

                var maxVolume = prevDatesDataList.Max(x => x.Volume);
                if (maxVolume == 0)
                {
                    entry.CalculatedFields.VolumeAlert2 = "missingdata";
                    updatedEntries++;
                ...
                var volPercent = entry.Volume / maxVolume;
                if (volPercent is >= (decimal)1.2 and <= (decimal)1.6 && entry.CalculatedFields.UpToday == true)
                {
                    entry.CalculatedFields.VolumeAlert2 = volPercent.ToString("P2");
                    updatedEntries++;
                    continue;
                }

                entry.CalculatedFields.VolumeAlert2 = "false";
                updatedEntries++;
            }
            catch (Exception ex)
            {
