...
{
    public async Task CheckSendVolumeAlert(DateTime targetDate, CancellationToken cToken)
    {
        const AlertType alertType = AlertType.Volume;
        const string headerRow = "Sector,Industry,Category,Symbol,VolPercent";
        ...
        try
        {
            ...
            tempFile.AddRange(alertData.Select(entry => $"{entry.IndustryInfo!.Sector},{entry.IndustryInfo.Industry},{entry.IndustryInfo.Category},{entry.Symbol},{entry.CalculatedFields!.VolumeAlert}"));
            await _discordClient.SendFileAsync(DiscordChannel.VolAlert, tempFile, targetDate, cToken);
        }
        catch (Exception ex)
        {
